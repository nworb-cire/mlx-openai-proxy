...
def test_metrics_store_preserves_audio_input_metadata(tmp_path: Path) -> None:
    store = MetricsStore(str(tmp_path / "metrics.db"))
    request_id = store.start_request(
        {
            ...
            "input_chars": 0,
            "input_image_count": 0,
            "input_audio_seconds": 1.25,
        }
    )

    active = store.get_active_requests()
    assert active[0]["path"] == "/v1/audio/transcriptions"
    assert active[0]["input_audio_seconds"] == 1.25

    store.complete_request(request_id, output_chars=5)
    history = store.get_history(limit=1)
    assert history[0]["path"] == "/v1/audio/transcriptions"
    assert history[0]["input_audio_seconds"] == 1.25
